...
SimpleCombatLog = AceLibrary("AceAddon-2.0"):new("AceConsole-2.0", "AceHook-2.1", "AceDebug-2.0")
...
function SimpleCombatLog:OnCombatEvent(event, info)
	...
	elseif info.type == 'unknown' then
		self:Debug("Unknown message: ", event, info.message)
		return
	else
		msgType = infoType
	end
...
function SimpleCombatLog:ParseInfo(id, msgType, tokens)
	...
	-- Recycle the cache.
	for k in pairs(cache2) do
		cache2[k] = nil
	end
	
	local myTokens = cache2
...
function SimpleCombatLog:GetElementType(element)
	for elementType, keyword in pairs(self.fixCombatLog) do
		if element == keyword then
			return elementType
		end
	end

	self:LevelDebug(2, "Unknown element:", element)
	return 'physical'
end
