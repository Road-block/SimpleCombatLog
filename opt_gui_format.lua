...
function SimpleCombatLog:ConvertFormat(...)
	--local arg = { ... }
	arg[1] = string.gsub(arg[1], "|", "||") -- "add slash" so that users can see color codes.	
	for i=2, arg.n do
...
local function UpdateFormatOptionsTable(self, id, level, value)
	...
	elseif level == 2 and value == 'format' then
		...
		for i, group in pairs{ "Combat", "Spell", "Misc", "Trailer" } do
			dewdrop:AddLine(
				'text', L[group],
				'hasArrow', true,
				'value', group
	...
	elseif level == 3 and self.currMenuType == 'format' then
		...
		for i, v in pairs(self.formatList[value]) do
			if v ~= "" then
				msgType = v
